async function getFilmesDetalheIngles(idFilme) {
    ...
    const res = await fetch(`https://api.themoviedb.org/3/movie/${idFilme}?language=en-US`, cabecalho)
    if (!res.ok) {
...
export default getFilmesDetalheIngles;