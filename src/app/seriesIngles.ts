async function getSeriesDetalheIngles(idSerie) {
    ...
    const res = await fetch(`https://api.themoviedb.org/3/tv/${idSerie}?language=en-US`, cabecalho)
    if (!res.ok) {
...
export default getSeriesDetalheIngles;